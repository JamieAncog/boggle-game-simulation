...
{
    HASH_INDEX_T h = this->probe(key);
    if((npos == h) || nullptr == table_[h] || table_[h]->deleted == true){
        return nullptr;
    }
...
{   
    if (mIndex_ >= 28-1){
        throw std::logic_error("Beyond capacity");
    }
    mIndex_++;
    std::vector<ItemType> addBack;
    ...
    for (int i = 0; i < (int) addBack.size(); i++){
        insert(addBack[i]);
    }
}
